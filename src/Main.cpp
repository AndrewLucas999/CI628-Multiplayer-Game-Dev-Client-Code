    ...
    while (is_running) {
        ...
        while (SDL_PollEvent(&event)) {
            if ((event.type == SDL_KEYDOWN || event.type == SDL_KEYUP) && event.key.repeat == 0) {
                ...
                // Send disconnect to server to fully exit the program
        ...
        // 1 Frame = 0.017 seconds / 17 milliseconds / 1/60 seconds
        SDL_Delay(17);
    }
}
...
int main(int argc, char** argv) {
    ...
    // Handle incorrect file file type
    if ((IMG_Init(IMG_INIT_PNG) & IMG_INIT_PNG) != IMG_INIT_PNG) {
        printf("File type not PNG");
        exit(4);
    }
    
    // Handle font cannot be loaded
    if (TTF_Init() < 0) {
